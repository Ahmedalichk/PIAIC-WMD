...
{
  {
    ...
    for (let i = 0; i < names.length; i++) {
      console.log(
        `Hello my good friend ${names[i]}. How are you, I hope all going good!!!`
      );
    }
  }
}
...

{
  ...
  for (let i = 0; i < guestList.length; i++) {
    console.log(
      `Hey ${guestList[i]}, I would like to invite you for dinner, it will be my pleasure if you come.`
    );
  }
}
{
  ...
  guestList.forEach((guestName) => {
    console.log(
      `Dear ${guestName}, you are invited for dinner, please join us`
    );
  });
}
...

{
  ...
  console.log(
    "invitation for dinner is just for two people due to some issues"
  );
  ...
  while (guestList.length > 2) {
    ...
    console.log(
      `Dear ${removeGuest}, we are sorry we cannot invite you for dinner due to some issues occured`
    );
  }
...

{
  ...
  let cars: Array<typeof car> = []; // Array of object
...
{
  function Car(name: string, company: string, model: string, color: string) {
    return {
      name,
      company,
      model,
...
{
  let favorite_fruits: string[] = ["mango", "melon", "kiwi"];
...
{
  let userName: string[] = ["ali", "hassan", "admin", "john", "david"];
...

{
  ...
  let new_users: string[] = ["khizer", "babar", "absar", "moin", "asif"];
  console.log("\n\n");
  let lowerArr: string[] = [];
  ...
  for (let i of lowerArr) {
    ...
    if (taken) {
      console.log(
        `You need to enter a new username, because ${i} already taken`
      );
    } else {
...
{
  console.log("\n");
  const commonCharacteristic: string = "friendly";
  ...
  console.log(
    `Any of these animals would make a great pet because they are ${commonCharacteristic}.`
  );
}
...
function storeCarInfo(manufacturer: string, modelName: string, ...options: { [key: string]: any }[]): Car {
    const car: Car = {
        manufacturer,
        modelName
    };
